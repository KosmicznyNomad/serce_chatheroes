        ...
        def create_window():
            ...
            return sg.Window('Multistep Prompting Chatbot', layout, finalize=True)
        ...
        def process_query(window, query):
            window['-STATUS-'].update('Przetwarzanie...')
            window['-CHAT-'].print(f"\nTy: {query}")
            window['-QUERY-'].update('')
            window.refresh()
        ...
        def main():
            ...
            while True:
                ...
                if event == 'Wyślij' and values['-QUERY-'].strip():
                    threading.Thread(target=process_query, args=(window, values['-QUERY-']), daemon=True).start()
